...
- **プロジェクションマッピング的な出力補正**＝**Output Surface（出力変形）**。最終段にもう一段
  メッシュ/コーナーピン補正を掛ける。**正式機能（M10・`08`/`11` B9）だが現状は未実装**（`OutputManager` に TODO）。
...
- **投影補正（Output Surface）**：完成映像“全体”をプロジェクタ面に合わせる出力段の変形＝**M10・未実装**（`08`/`11` B9）。
  使う技術（コーナーピン＋多pin メッシュワープ）は **`Compositor` で実装済**だが、そこでの用途は**埋め込み領域（Input Surface）**。
  M10 では同じ技術を**出力段に流用**＋**エッジブレンド/フェザー・マスク**で MadMapper 相当のマッピング品質を担保。
