...
- **プロジェクションマッピング的な出力補正**＝**Output Surface（出力変形）**。最終段にもう一段
  メッシュ/コーナーピン補正を掛ける。**M10 実装済**（`OutputWarp`＋`OutputManager` 配線）。
  **編集UI も実装済（#25）**: オペレータUI の WARP エディタに **EMBED ⇄ OUTPUT** 切替を追加し、OUTPUT 側で
  `OutputWarp` の四隅/メッシュを直接ドラッグ編集できる。OUTPUT 編集中はプレビューを変形後 RT に切替（WYSIWYG）。
  配線: シーンに `OutputWarp` を追加し `OutputManager.Output Warp`(`_outputWarp`) に割当（未割当なら素通し）。
  **今後（#35）**: OUTPUT 編集に**格子オーバーレイ常時表示**＋**グリッド/テストパターンを実際に投影する校正トグル**
  （会場でプロジェクタを物理面に合わせる定番手法：投影→整列→隠す）。メッシュ微調整は Grid と同じ **Bezier**（#34）で滑らかに。
  ＝「Mask/Crop」ではなく「**素の投影＋グリッドで微調整**」という運用（出力は完成フレーム全体をプロジェクタ面へワープする段）。
