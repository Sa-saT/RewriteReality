...
### M0. 環境・依存（土台）※完了（最新の進捗は `CLAUDE.md`「現在の状況」が正本）
...
- プロジェクタ台形補正（**出力段コーナーピン＝4頂点ワープ）を入れる**（→ B9/M10 の **Output Surface（出力変形）**に内包・拡張・未実装）。
...
- **song の動画だけライブ速度可変【確定 2026-06-30】**：本番中、song の**映像のみ**再生速度をインタラクティブに変える
  （`SourceVideo.PlaybackSpeed`＝VideoPlayer.playbackSpeed・**ボタン/スクロール(ジョグ)割当**・0=フリーズ〜Nx・逆再生は将来）。
  音声トラックは通常速度のまま（意図的なズレ）。四隅は video time キーで自動整合。
...
- B は確定済み。A の各 M を上から実装していく。**現在地・次の一手は `CLAUDE.md`「現在の状況／次の一手」が正本**
  （M0 完了済み・コア実装中。AV ショー化拡張 M10〜M13 は B9）。
