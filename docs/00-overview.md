...
| 埋め込み方式 | **マーカー/トラッキング追従** | 動く対象にカメラ映像を貼る。ArUco マーカー優先＋特徴点トラッキング補助 |
...
| マーカー/ホモグラフィ | **OpenCvSharp**（ArUco + findHomography） | shimat | ◎ OSS（BSD） |

> 有料アセット（OpenCV for Unity 等）は使わない。CV は無料の **OpenCvSharp** に集約。
> 唯一の技術リスクは OpenCvSharp のネイティブライブラリを Apple Silicon で通すこと（`02`/`03` で対策）。
...
難所は (1) **OpenCvSharp の Apple Silicon ネイティブ導入**、(2) **トラッキングのロバスト性**、
(3) **60fps を維持する GPU/GC 最適化**の3点。対策は各章に記載。
