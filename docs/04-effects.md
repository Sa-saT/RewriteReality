...
### 5. （将来オプション）深度ドリブン — 深度カメラ / `IDepthSource`
...
> 取得経路: **深度カメラ（深度センサー）**から色＋深度を受ける。`IDepthSource` で差し替え可能。
> 接続は ① USB 直結（各社 SDK で色＋深度を Texture 化）② NDI 経由（送出機→Unity が NDI-in で受信）のいずれか。
> 候補センサー: **Orbbec Femto Bolt/Mega（Apple Silicon 向け SDK・Unity 連携あり＝macOS では本命）／Intel RealSense D4xx**。
> 旧案の **iPhone Pro LiDAR（Rcam3 方式・ARKit sceneDepth → NDI）は Pro 機がある場合の参考実装**（本プロジェクトでは非前提・iPhone/iPad Pro 縛りは撤回）。
> 限界: センサー依存（概ね 〜数 m・低解像度・エッジノイズ）→ エフェクト/オクルージョン向き（精密計測は不向き）。
> 機種選定（USB or NDI・解像度・arm64 SDK）は **M9 着手時に確定**。
