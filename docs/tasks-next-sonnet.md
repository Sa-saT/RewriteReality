...
## Task 1（#28a）: タイムライン音声の内部再生 — `SourceAudio` シンク＋バインド
...
## Task 2（#28b）: トラック mute の実効化＋最終ミックス解析の整合
...
## Task 3（#36）: ショーデータ永続化 — バンクの JSON 保存/読込
...
## Task 4（#37）: PERFORM 左ドック・ライブラリの実データ連動（最小版）
