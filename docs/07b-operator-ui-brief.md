...
### 3.2.1 Fit モード（実装済＋今後の方向・2026-07-03）

Input Surface は **2 系統**（`Surface.FitMode`）。射影2×2（旧 Project）は廃止し、歪ませる面は Grid に一本化する。

- **Mask（既定・歪めない窓抜き・実装済 #32）**: content を全面等倍で置き、Surface 形状で窓抜き（枠内=映る/枠外=映らない）。
  四隅/pin を動かしても内容は歪まない。**SHAPE**=窓の形/移動/Scale、**CONTENT**=枠内映像の pan/Zoom（ドラッグ）。
- **Grid(Bezier)（歪ませる・#34）**: MadMapper GridGenerator 手本。格子面を当て、**内蔵テストパターン**で傾きを見ながら
  ノードを合わせ→**カメラ/動画へ差替**。ワープは **Bezier**で滑らか。既定解像度↑。故意の局所歪みはエフェクト（#33）。
- **Output（#25 実装済＋#35）**: 完成フレーム全体をプロジェクタ面へ変形。素の無変形＋微調整、**格子オーバーレイ＋
  グリッド投影校正**（#35）。「Mask/Crop」ではなく「素の投影＋グリッド微調整」。
