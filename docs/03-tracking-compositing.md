...
### 今後の方向：Grid/Bezier モードで「歪ませる面」を一本化（#34・決定 2026-07-03）

**FitMode.Project（射影2×2）は廃止し、「歪ませる面」は Grid(Bezier) に一本化**する（MadMapper GridGenerator 手本）。
- ワークフロー: ①格子線入りの面を Mapping 先へ当てる → ②内蔵**テストパターン（グリッド/チェッカー）**を content に
  貼って**斜めの傾きを見ながらノードを合わせる** → ③**カメラ/動画へ差し替え**。
- ワープは **Bezier**（現状の線形 bilinear のカクつき解消・滑らかな“程よい歪み”）。`WarpMath` に Bezier 評価を追加し、
  Compositor はパッチを細分化して三角形化。WarpCanvas は Bezier 制御点＋接線ハンドル＋**常時グリッド表示**。既定解像度↑。
- 整理後の Input Surface＝**Mask（歪めない窓・既定）／Grid(Bezier)（意図的に程よく歪める・微調整）**の2系統。
- 故意の局所歪み（映像内パペットpin）は**エフェクト**（#33）で別途。出力側のグリッド校正は #35（`06`）。
