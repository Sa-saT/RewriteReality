...
### 同期の正確性＝リポジトリ内ミラーで担保【確定 2026-07-11】

ClaudeDesign の変更を取りこぼさないため、`docs/design-sync/operator/` にリモート
（`ui_kits/operator/`）の**ミラー（スナップショット）**を保持する。同期は「リモート取得 →
ミラーと diff → UXML/USS/C# へ反映 → ミラー更新を同コミットに含める」で行う。
`.md` に書かれない変更が jsx 側にだけ入ることがあるため、**App.jsx（シェル全体）を毎回照合**する。
手順・運用ルールの正本＝ `docs/design-sync/README.md`。
